...
The screen is the in-app "Lili match timeline" intel hub. Its `TABS` array is **one tournament-level tab plus seven match-intelligence tabs**, each rendered by its own explicit branch in `MatchHeatmapScreen` and backed by an honest model in `lib/` (refreshed recon #40, 2026-06-20).

**PRODUCT RULE (architecture invariant — preserve it):**
> **Dashboard = tournament-level intelligence.** **Overview / Heatmap / Home Edge / Attack Zones / Shots / Pass Map / Players = focused match intelligence or signal views.** Never put match-level widgets in Dashboard, and never push tournament-level rollups into the match tabs.
...
1. **📊 Overview** — `OverviewModule` (+ `TournamentImpactPanel`) ← `lib/matchOverview.ts` · "why did this team win & what does it mean for the WC?": stat pairs (incl. its own KEY STATISTICS card), control %, verdict, drivers, events, standings/qualification impact.
2. **Heatmap** — `TerritoryPitch` + `MomentumPanel` ← territory share (`lib/heatmap.ts` `.share`) + `lib/matchMomentum.ts` · **smooth SVG territory-control map** (rebuilt 2026-06-20 on the shared `components/PitchSvg.tsx`): a left↔right gradient — RED = away's end (left), BLUE = home's end (right), PURPLE contested middle; the contested band shifts with territory share. Markings render ABOVE the gradient. Below it the smooth `MomentumWave` (home events above the line, away below, leader-lines to the exact minute). Rail order: Lili Insight → How to Read → Territory Share → Team Stats. A normal tab like the others — its `HeatmapBody` is shared with the pre-kickoff **forecast** branch (forecast swaps the momentum wave for the forecast note). Desktop uses the shared edge-to-edge two-column grid (`heatLeft flex 1.3` + `heatRight width 330`), phone is single-column scroll. Honest model (possession/shots/xG, not tracking), laterally symmetric.
...
4. **⚔️ Attack Zones** — `AttackZonesPanel` (SVG) + `AttackZonesModule` ← `lib/attackZones.ts` · "where does this team create danger?": only bands we truly have — inside box, from distance, wide/corners. No invented left/right split.
5. **🎯 Shots** — `ShotsMapPanel` (SVG) + `ShotsModule` ← `lib/shotsModel.ts` · shot splits, conversion, xG, Danger Index, finishing efficiency, GK saves vs xGA. Plotted by **area**, never fake coordinates.
6. **🕸 Pass Map** — `PassMapPanel` (SVG) + `PassMapModule` ← `lib/passStructure.ts` · real per-player passes/accuracy + possession, laid out by formation role. **No fabricated who→whom links.** Connectivity Score, Team Style.
...
The SVG panels (`AttackZonesPanel` / `ShotsMapPanel` / `PassMapPanel` / `MomentumPanel`) live in `components/MatchDashboard.tsx` and render **inside their own per-match tabs** — they are NOT in the Dashboard tab. (`KeyStatsPanel` was removed in recon #40: it duplicated OverviewModule's own KEY STATISTICS card.)
