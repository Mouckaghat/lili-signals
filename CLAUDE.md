...
2. **Heatmap** — `Pitch` + `Momentum` ← `lib/heatmap.ts` + `lib/matchMomentum.ts` · **threat / "where was danger created?" map** (redesigned 2026-06-20), *modelled* from possession, shots in/out box, on-goal, corners, xG + formation prior (NOT optical tracking). Heat lives in football **zones** with a fixed **danger hierarchy** — penalty area ≫ half-spaces > wide channels > midfield — gated hard behind the halfway line so the own half/midfield stay cold (no "weather-radar" half-glow). **Laterally MIRROR-SYMMETRIC** (LW=RW, LHS=RHS): we have no x-y data, so we model central-vs-wide, **never left-vs-right**. Blue=home (attacks right), red=away (attacks left); each side's heat sits in its own attacking third so purple/contested is now rare. Pitch markings are drawn bright & sharp **above** the heat (incl. clipped penalty arcs); low web blur (2.5px). Storytelling test: pitch alone must answer who attacked more / where / how dangerous / central-or-wide in 3s.
