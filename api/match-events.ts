...
// handoff (the baked events that would cover them are deploy-gated via [skip ci]).
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // so the Momentum goal/card markers don't freeze at full time (the baked
    // events are deploy-gated and won't reach the client until the next deploy).
