...
function generateFile(group: MatchStats[], knockout: MatchStats[], updatedAt: string): string {
...
async function main() {
  ...
  // Group-stage AND knockout fixtures both get a match-intelligence model. The
  // knockout ties live in their own array (WC_KNOCKOUT); include them here or
  // their stats are silently dropped (the bug that left "Relive the match" dark).
  // They're written to a SEPARATE export (KNOCKOUT_MATCH_STATS) below so the
  // tournament aggregates that iterate MATCH_STATS aren't skewed by KO games.
  const wcByKey = new Map(WC_FIXTURES.map((f) => [`${f.home}|${f.away}`, f]));
  const koByKey = new Map(WC_KNOCKOUT.map((f) => [`${f.home}|${f.away}`, f]));
  const koIds   = new Set(WC_KNOCKOUT.map((f) => f.id));
  const order   = new Map([...WC_FIXTURES, ...WC_KNOCKOUT].map((f, i) => [f.id, i]));

  const played = fixtures
    .map((f) => {
      const key = `${normTeam(f.teams.home.name)}|${normTeam(f.teams.away.name)}`;
      return {
        apiId: f.fixture.id, short: f.fixture.status.short, elapsed: f.fixture.status.elapsed,
        wc: wcByKey.get(key) ?? koByKey.get(key),
      };
    })
  ...
  const groupEntries = entries.filter((e) => !koIds.has(e.fixtureId));
  const koEntries    = entries.filter((e) => koIds.has(e.fixtureId));
  console.log(`  ✓  ${entries.length} matches with stats (${groupEntries.length} group, ${koEntries.length} knockout)`);
  ...
  const _wrote = writeGeneratedFile(OUT_PATH, generateFile(groupEntries, koEntries, now));
