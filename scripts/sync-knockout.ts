...
interface KOEntry {
  ...
  penHome: number | null;   // penalty-shootout score (null unless the tie went to spot-kicks)
  penAway: number | null;
}
...
function build(apiFixtures: ApiFixture[]): KOEntry[] {
  ...
  for (const af of apiFixtures) {
    ...
    out.push({
      ...
      penHome:   af.score?.penalty?.home ?? null,
      penAway:   af.score?.penalty?.away ?? null,
    });
  }
  out.sort((a, b) =>
...
function generateFile(rows: KOEntry[], updatedAt: string): string {
  const body = rows.map((r) =>
    ...
    `winner: ${r.winner === null ? 'null' : `'${r.winner}'`}, penHome: ${n(r.penHome)}, penAway: ${n(r.penAway)} },`
  ).join('\n');
